#              PyWeightTracker.py - Weight Tracking Software

#  File Name: PyWeightTracker.py
#  Description: Weight tracking software
#  Author(s): Electro L.I.B (2023)
#  Optimization: ChatGPT2
#  Version: 1.0
#  License: GPLv3

#  PyWeightTracker.py is free software: you can redistribute it and/or modify
...
class PyWeightTrackerApp:
    ...
    def __init__(self, root):
        ...
        self.goal_label = tk.Label(self.root, text="Weight goal (in pounds):")
        self.weight_label = tk.Label(self.root, text="Weight (pounds):")
    ...
    def create_controls(self):
        ...
        unit_label = tk.Label(unit_frame, text="Unit of measurement:")
        unit_label.grid(row=0, column=0)

        lbs_radio = tk.Radiobutton(unit_frame, text="pounds", variable=self.unit_var, value=1, command=self.unit_selected)
        lbs_radio.grid(row=0, column=1)

        kg_radio = tk.Radiobutton(unit_frame, text="kilograms", variable=self.unit_var, value=2, command=self.unit_selected)
        ...
        self.date_button = tk.Button(add_weight_frame, text=' Select a date ', command=self.choose_date)
        self.date_button.grid(row=0, column=2)

        goal_button = tk.Button(add_weight_frame, text="Save the goal", command=self.save_goal)
        goal_button.grid(row=2, column=2, columnspan=2)
        
        self.goal_label = tk.Label(add_weight_frame, text="Weight goal (pounds):")
        ...
        self.weight_label = tk.Label(add_weight_frame, text="Weight (pounds):")
        ...
        add_button = tk.Button(add_weight_frame, text="         Add       ", command=self.add_weight)
        add_button.grid(row=1, column=2, columnspan=2)

        show_button = tk.Button(self.root, text="Refresh Display", command=self.show_plot)
        show_button.pack()

        clear_button = tk.Button(self.root, text=" Delete all data ", command=self.clear_data)
        clear_button.pack()
    ...
    def unit_selected(self):
        if self.unit_var.get() == 2:
            self.weight_label.config(text="Weight (in kilograms):")
            self.goal_label.config(text="Weight goal (in kilograms):")
        else:
            self.weight_label.config(text="Weight (in pounds):")
            self.goal_label.config(text="Weight goal (in pounds):")
    ...
    def update_plot(self):
        ...
        weights = [data[1] for data in sorted_data if isinstance(data[0], datetime.date)]
        self.ax.plot(dates, weights, marker='o', label='Data')
        ...
        if goal_data:
            ...
            if goal is not None:  # Vérifiez si l'objectif n'est pas None
                self.ax.axhline(y=goal, color='red', linestyle='--', label='Goal')

        self.ax.set_title('The evolution of weight')
        self.ax.set_xlabel('Date')
        if self.unit_var.get() == 2:
            self.ax.set_ylabel('Weight (in kilograms)')
        else:
            self.ax.set_ylabel('Weight (in pounds)')
    ...
    def clear_data(self):
        confirmation = mbox.askokcancel("Confirmation: Are you sure you want to erase all data?")
        if confirmation:
            self.weight_data = []
            self.save_weight_data_to_file('weight_data.csv')
            self.update_plot()
        ...
        if event.xdata is not None and event.ydata is not None:
            ...
            for i, (date, weight) in enumerate(self.weight_data):
                if date == clicked_date:
                    confirmation = mbox.askokcancel("Confirmation", f"Are you sure you want to delete the record of the {date}?")
                    if confirmation:
